//! Music/SFX channels and master-volume application.
//!
//! Reads [`GameSettings`] (from `jam-settings`) and applies it to Godot's
//! `AudioServer`; plays sound effects folding in the per-sound and SFX gains.
...
use jam_core::{Gain, GameState, Volume};
use jam_settings::GameSettings;
...
impl Plugin for AudioPlugin {
    fn build(&self, app: &mut App) {
        ...
        app.configure_loading_state(
            LoadingStateConfig::new(GameState::Loading).load_collection::<GameAudio>(),
        )
...
pub struct GameAudio {
    #[asset(path = "addons/jamkit/audio/test_sound.wav")]
